...
- **Types, Warnings & Linters:** NEVER use hacks like disabling or suppressing warnings, bypassing the type system (e.g.: unchecked casts), or employing "hidden" logic (e.g.: reflection, unsafe pointers) unless explicitly instructed to by the user. Instead, use explicit and idiomatic language features (e.g.: type guards, explicit construction, or interface assertions) that maintain structural integrity and type safety.
- **Design Patterns:** Prioritize explicit composition and delegation (e.g.: wrapper types, interfaces, or factory functions) over complex inheritance or prototype-based cloning. When extending or modifying existing types, prefer patterns that are easily traceable and type-safe.
